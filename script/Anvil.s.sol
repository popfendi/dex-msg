...
contract DexMsgScript is Script, DeployPermit2 {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(DexMsg).creationCode, abi.encode(address(manager)));

        // ----------------------------- //
        // Deploy the hook using CREATE2 //
        // ----------------------------- //
        vm.broadcast();
        DexMsg dexMsg = new DexMsg{salt: salt}(manager);
        require(address(dexMsg) == hookAddress, "DexMsgScript: hook address mismatch");
        ...
        vm.startBroadcast();
        testLifecycle(manager, address(dexMsg), posm, lpRouter, swapRouter);
        vm.stopBroadcast();
    }
